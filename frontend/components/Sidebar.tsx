...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`glass-lg backdrop-blur-2xl bg-white/[0.06] border-r border-white/[0.1] transition-all duration-300 flex flex-col ${
        isOpen ? 'w-64' : 'w-20'
      }`}
    >
      {/* Logo/Brand */}
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all duration-200 text-sm font-light ${
                active
